...
class Dashboard(ctk.CTkFrame):
    def __init__(self, parent, **kwargs):
        ...
        ctk.CTkLabel(self.most_used_frame, text="Most Used Snippets", font=ctk.CTkFont(size=16, weight="bold")).pack(
            pady=10, anchor="w", padx=15
        )
        ...
        ctk.CTkLabel(self.chart_frame, text="Weekly Expansion Chart", font=ctk.CTkFont(size=16, weight="bold")).pack(
            pady=10, anchor="w", padx=15
        )
        ...
        self.chart_canvas = ctk.CTkCanvas(
            self.chart_frame,
            height=180,
            bg="#1e1e1e" if ctk.get_appearance_mode().lower() == "dark" else "#e5e5e5",
    ...
    def create_card(self, parent, title: str, value: str, col: int) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(card, text=title, font=ctk.CTkFont(size=13, weight="bold"), text_color="gray").pack(
            pady=(15, 2)
        )
    ...
    def refresh_stats(self):
        ...
        for widget in self.most_used_container.winfo_children():
            widget.destroy()
    ...
    def draw_chart(self, stats: dict):
        ...
        for i in range(6, -1, -1):
            ...
            values.append(daily_stats.get(day.strftime("%Y-%m-%d"), 0))
        ...
        for i, (day, value) in enumerate(zip(days, values)):
            ...
            bar_height = (value / max_val) * graph_height
            y0 = height - 30 - bar_height
            x1 = x0 + bar_w
            y1 = height - 30

            self.chart_canvas.create_rectangle(x0, y0, x1, y1, fill="#3b82f6", outline="")
            self.chart_canvas.create_text((x0 + x1) / 2, height - 15, text=day, fill=text_color, font=("Consolas", 10))
            self.chart_canvas.create_text((x0 + x1) / 2, y0 - 10, text=str(value), fill=text_color, font=("Consolas", 8))
