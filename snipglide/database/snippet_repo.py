...
def row_to_snippet(row) -> Snippet:
    ...
    cols = row.keys()
    snippet_type = row["snippet_type"] if "snippet_type" in cols and row["snippet_type"] else "Text"
    ...
    return Snippet(
        ...
        language=row["language"] if "language" in cols and row["language"] else "Plain Text",
    ...
    try:
        ...
        cursor.execute(
            ...
            SELECT id, shortcut, substr(replacement, 1, 500) AS replacement,
                   group_id, tags, description, language, snippet_type, enabled, favorite,
        ...
        with conn:
            ...
            cursor.execute("""
                INSERT INTO snippets (
                    shortcut, replacement, group_id, tags, description, language, snippet_type,
            ...
            """, (
                ...
                snippet.description, snippet.language, getattr(snippet, "snippet_type", "Text"),
                int(snippet.enabled), int(snippet.favorite),
